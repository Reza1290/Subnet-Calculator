...
ip =  "192.168.17.0" #str(input("Net IP : "))

sub  =  24 #int(input("Subnet : "))

host = 60 #int(input("Host : "))
...
def generate_interval(base_ip, interval, jumlah):
    ...
    for i in range(jumlah):
        ...
        for j in range(4):
            if(interval_parts[j] * x >= 256):
                base_ip_parts[j-1] = base_ip_parts[j-1] + 1
                x = 0
                subnet_parts[j-1] = str(int(subnet_parts[j-1]) + 1)
                
            subnet_parts.append(str(base_ip_parts[j] + interval_parts[j] * x))
...
# ip_address = ip
ip_address = '192.168.17.0'
ip_address = bitwise_and_ip(ip_address, interval)

subnets = generate_interval(ip_address, interval_r, 8) #param terakhir = Jumlah Print
